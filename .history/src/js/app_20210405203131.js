...
if(!results)
{
  ...
  $(".getOpenBoth").on( "click", function() {
    ...
    $("#get_air_here").show();
    gsap.from("#get_air_here", {top: '-100vh', opacity:0, duration: .1, ease: "Elastic.easeOut" });
...
/*  */
  ...
  function moveToScreen07() {
    ...
    if ($(window).width() < 768) {
      gsap.from("#getOpen", {top: '100px', opacity:0, duration: .8, delay:2, ease: "Strong.easeOut" });
      playAni();  
   }
   else {
    gsap.to("#headlinePageSeven", {top: '0', opacity:1, duration: .1, ease: "Strong.easeOut" });
    gsap.from("#getOpen", {top: '100px', opacity:0, duration: .8, delay:3, ease: "Strong.easeOut" });
   }
